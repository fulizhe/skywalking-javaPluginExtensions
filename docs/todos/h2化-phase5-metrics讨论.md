...
>
> **⚠️ 2026-09-24 路线调整（本文部分被取代）**：本文 §1 / §3 / §9 主张的"**合并视图事件**挂钩"与"复用 `TraceEvaluator`"**已被取代**——Phase 5 改为 **入口段 a1 + H2 内存模式 + 多分辨率（分钟 + 小时）**，且不改告警包。**唯一权威 spec：`.scratch/h2-metrics/spec.md`（ready-for-agent）**；统计口径以 [`h2化-phase5-metrics-口径与插入点讨论.md`](h2化-phase5-metrics-口径与插入点讨论.md) 的 a1 为准。
