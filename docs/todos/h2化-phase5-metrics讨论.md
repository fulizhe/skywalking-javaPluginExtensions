...
> **实现备注**：部分实现：当前指标聚合、H2 多分辨率存储/查询与 Facade 已落地；本文原合并视图挂钩、复用 `TraceEvaluator`、`maxDurationMs` 等方案未全部实现或已被取代。
>
> **文档定位**：基于 `docs/todos/h2化-统一方案.md`（§4 Phase 5、§5.3、§5.4、§6.3、§7.1、§9、§11）与当前代码的**独立讨论稿**；原讨论阶段标注为“未实现”，当前落地范围见上方备注。
>
> **已细化**：[`h2化-phase5-metrics实现细化.md`](h2化-phase5-metrics实现细化.md) —— 落到当前代码、逐项锁定决策（本文 §11 的 a–g 及其新增项）。冲突处以细化稿为准。
>
> **⚠️ 2026-09-24 路线调整（本文部分被取代）**：本文 §1 / §3 / §9 主张的"**合并视图事件**挂钩"与"复用 `TraceEvaluator`"**已被取代**——Phase 5 改为 **入口段 a1 + H2 内存模式 + 多分辨率（分钟 + 小时）**，且不改告警包。**唯一权威 spec：`.scratch/h2-metrics/spec.md`（ready-for-agent）**；统计口径以 [入口段口径讨论](<[已实现]h2化-phase5-metrics-口径与插入点讨论.md>) 的 a1 为准。
